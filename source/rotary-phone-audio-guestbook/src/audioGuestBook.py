...
class AudioGuestBook:
    ...
    def setup_record_greeting(self):
        ...
        self.record_greeting.when_pressed = self.pressed_play_random_message
    ...
    def play_random_message(self):
        """
        Plays the random_message_audio
        """
        try:
            recordings_path = Path(self.config["recordings_path"]) 
            if recordings_path.exists() and recordings_path.is_dir():
                ...
                if files_only:
                    ...
                    logger.info(f"Randomly selected file: {random_file.name}")
                else:
                    logger.info("No files found to select randomly.")
